...
const AdminDashboardPage = () => {
  ...
  // Accept the incoming call (one-on-one or conference) and navigate to the video call screen
const handleAccept = async (roomID) => {
  ...
  try {
    ...
    if (callSnapshot.exists()) {
      ...
      const isConferenceCall = callData.isConferenceCall === true;

      // Update call status to "in-progress" if it's still pending
      ...
      // If this call was forwarded from another admin, mark the original admin as available
      if (callData.forwardedFrom) {
        await update(ref(db, `JPMCReceptionistAdmin/${callData.forwardedFrom}`), { status: "available" });
      }

      setIsAvailable(false);
      await update(ref(db, `available/${userIdRef.current}`), { status: "unavailable" });

      // After a brief delay, navigate to the video call page with the conference flag
      setTimeout(() => {
        setIsConnecting(false);
        navigate(`/video-calling-admin/${roomID}`, {
          state: { roomID, adminId: userIdRef.current, isConferenceCall }
        });
      }, 3000);
    }
  } catch (error) {
      ...
      <nav className="bg-gradient-to-t from-[rgb(12,25,97)] to-[rgb(12,25,97)]/5 backdrop-blur-md border border-white/20 shadow-lg">
        <div className="container flex items-center justify-between px-6 py-3 mx-auto">
          <div className="h-12 flex items-center">
            <img className="h-7 filter invert brightness-0" src={img} alt="Purview Logo" />
          </div>
